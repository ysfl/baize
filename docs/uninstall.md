...
| 本机连接程序 | 尝试卸载当前宿主机上的连接程序、登录声明和固定镜像更新组件 |
| 更新历史 | 卸载前把已有请求、回执和日志保留到 `/var/lib/baize/image-upgrade-history/` |
| 容器 | 停止并移除白泽容器与网络 |
...
这会停止并移除 `baize-agent` 系统服务、安装目录、登录声明和固定镜像更新组件。已有镜像更新请求、回执和日志会先归档到 `/var/lib/baize/image-upgrade-history/`;其它被纳管服务器上的连接程序需要在对应服务器上分别卸载。
