...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --i-understand-no-backup)
      ...
      shift
      ;;
    --public-url|--agent-public-url|--web-api-base-url|--server-public-port|--web-public-port|--postgres-public-port|--redis-public-port|--server-target-arch|--deploy-mode|--server-image|--web-image|--version)
      [[ -n "${2:-}" ]] || die "$1 不能为空"
      INIT_ARGS+=("$1" "$2")
      shift 2
      ;;
