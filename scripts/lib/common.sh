...
baize_resolve_language() {
  local env_file="${1:-$BAIZE_ROOT_DIR/.env}"
  local configured="${BAIZE_LANG:-}"
  if [[ -z "$configured" && -f "$env_file" ]]; then
    configured="$(baize_read_env BAIZE_LANG "$env_file")"
  fi
  configured="$(baize_strip_env_quotes "$configured")"
  case "$configured" in
    en|en-US|en_US) printf '%s' "en" ;;
    zh|zh-CN|zh_CN|"") printf '%s' "zh" ;;
    *)
      echo "[baize] unsupported BAIZE_LANG=$configured, fallback to zh" >&2
      printf '%s' "zh"
      ;;
  esac
}
...
baize_resolve_stack_mode() {
  local env_file="${1:-$BAIZE_ROOT_DIR/.env}"
  local configured
  configured="${BAIZE_STACK_MODE:-$(baize_read_env BAIZE_STACK_MODE "$env_file")}"
  configured="$(baize_strip_env_quotes "$configured")"
  [[ -n "$configured" ]] || configured="full"
  case "$configured" in
    full) printf '%s' "full" ;;
    server-only|server_only|server) printf '%s' "server-only" ;;
    *)
      echo "BAIZE_STACK_MODE 仅支持 full、server-only，当前值: $configured" >&2
      return 1
      ;;
  esac
}
...
baize_has_build_artifacts() {
  ...
  if [[ -z "$stack_mode" ]]; then
    stack_mode="$(baize_resolve_stack_mode "$env_file")" || return 1
  fi
...
baize_resolve_deploy_mode() {
  ...
  case "$configured" in
    ...
    auto)
      stack_mode="$(baize_resolve_stack_mode "$env_file")" || return 1
      if baize_has_build_artifacts "$env_file" "$stack_mode"; then
        printf '%s' "build"
      else
        printf '%s' "image"
      fi
...
baize_require_build_artifacts() {
  ...
  arch="$(baize_required_server_arch "$env_file")"
  baize_has_build_artifacts "$env_file" "$stack_mode" && return 0
  cat >&2 <<EOF
...
baize_compose_profile_args() {
  local env_file="${1:-$BAIZE_ROOT_DIR/.env}"
  local stack_mode
  stack_mode="$(baize_resolve_stack_mode "$env_file" 2>/dev/null || printf '%s' full)"
  if baize_stack_has_web "$stack_mode"; then
    printf '%s\n' "--profile"
    printf '%s\n' "web"
  fi
...
baize_compose() {
  ...
  while IFS= read -r arg; do
    profile_args+=("$arg")
  done < <(baize_compose_profile_args "$BAIZE_ROOT_DIR/.env")
  docker compose "${BAIZE_COMPOSE_ARGS[@]}" "${profile_args[@]}" "$@"
}
