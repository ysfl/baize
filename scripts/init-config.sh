...
BAIZE_VERSION="${BAIZE_VERSION:-${BAIZE_SERVER_VERSION:-0.3.1}}"
SERVER_VERSION="${BAIZE_SERVER_VERSION:-$BAIZE_VERSION}"
WEB_VERSION="${BAIZE_WEB_VERSION:-1.2.0}"
...
IMAGE_UPGRADE_TIMEOUT_SEC="${BAIZE_IMAGE_UPGRADE_TIMEOUT_SEC:-900}"
...
选项:
  ...
  --version <version>            兼容版本别名，默认 0.3.1；等同于 --server-version
  --server-version <version>     中心服务镜像标签版本，默认 0.3.1
  --web-version <version>        控制台镜像标签版本，默认 0.3.1
  --server-image <image>         中心服务镜像名，默认 ghcr.io/ysfl/baize-server:0.3.1
  --web-image <image>            控制台镜像名，默认 ghcr.io/ysfl/baize-web:1.2.0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --stack-mode)
      STACK_MODE="${2:-}"
      [[ -n "$STACK_MODE" ]] || die "--stack-mode 不能为空"
      shift 2
      ;;
    --version)
      BAIZE_VERSION="${2:-}"
      [[ -n "$BAIZE_VERSION" ]] || die "--version 不能为空"
      SERVER_VERSION="$BAIZE_VERSION"
      shift 2
      ;;
    --server-version)
      SERVER_VERSION="${2:-}"
      [[ -n "$SERVER_VERSION" ]] || die "--server-version 不能为空"
      BAIZE_VERSION="$SERVER_VERSION"
      shift 2
      ;;
    --web-version)
      WEB_VERSION="${2:-}"
      [[ -n "$WEB_VERSION" ]] || die "--web-version 不能为空"
      shift 2
      ;;
    --server-image)
      SERVER_IMAGE="${2:-}"
...
[[ "$IMAGE_UPGRADE_TIMEOUT_SEC" =~ ^[0-9]+$ ]] || die "BAIZE_IMAGE_UPGRADE_TIMEOUT_SEC 必须是数字"
(( IMAGE_UPGRADE_TIMEOUT_SEC >= 60 && IMAGE_UPGRADE_TIMEOUT_SEC <= 3600 )) \
  || die "BAIZE_IMAGE_UPGRADE_TIMEOUT_SEC 必须在 60 到 3600 秒之间"
...
normalize_stack_mode() {
  case "$1" in
    full) printf '%s' "full" ;;
    server-only|server_only|server) printf '%s' "server-only" ;;
    *) return 1 ;;
  esac
}
...
validate_stack_mode() {
  if ! STACK_MODE="$(normalize_stack_mode "$1")"; then
    die "BAIZE_STACK_MODE 仅支持 full、server-only: $1"
  fi
}
...
validate_stack_mode "$STACK_MODE"
if [[ "$STACK_MODE" == "full" ]]; then
...
if [[ "$STACK_MODE" == "full" ]]; then
  port_items+=("WEB_PUBLIC_PORT=$WEB_PUBLIC_PORT")
fi
...
BAIZE_SERVER_VERSION=$SERVER_VERSION
BAIZE_WEB_VERSION=$WEB_VERSION
...
BAIZE_IMAGE_UPGRADE_TIMEOUT_SEC=$IMAGE_UPGRADE_TIMEOUT_SEC
