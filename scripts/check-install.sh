...
validate_distinct_ports() {
  local keys=("POSTGRES_PUBLIC_PORT" "REDIS_PUBLIC_PORT" "SERVER_PUBLIC_PORT")
  local i j left_key right_key left right
  if baize_stack_has_web "$stack_mode"; then
    keys+=("WEB_PUBLIC_PORT")
  fi
  for ((i = 0; i < ${#keys[@]}; i++)); do
    left_key="${keys[$i]}"
...
if [[ "$deploy_mode" == "build" ]]; then
  baize_require_build_artifacts "$ENV_FILE" "$stack_mode"
...
BAIZE_ROOT_DIR="$ROOT_DIR" baize_compose "$deploy_mode" --env-file "$ENV_FILE" config >/dev/null

log "静态安装检查通过，server_arch=$server_arch deploy_mode=$deploy_mode stack_mode=$stack_mode"
