...
选项:
  ...
  --skip-build                   不重新构建 server/web 镜像
  --skip-online-check            启动后跳过 HTTP 在线检查
  -h, --help                     显示帮助

English:
  Deploy PostgreSQL, Redis, Baize control service and console with Docker Compose.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --public-url|--agent-public-url|--web-api-base-url|--server-public-port|--web-public-port|--postgres-public-port|--redis-public-port|--server-target-arch|--deploy-mode|--server-image|--web-image|--version|--backup-dir)
      [[ -n "${2:-}" ]] || die "$1 不能为空"
      INIT_ARGS+=("$1" "$2")
...
if [[ "$SKIP_BUILD" == "1" ]]; then
  baize_compose "$DEPLOY_MODE" up -d server web
elif [[ "$DEPLOY_MODE" == "build" ]]; then
  baize_compose "$DEPLOY_MODE" up -d --build server web
else
  if ! baize_compose "$DEPLOY_MODE" pull server web; then
