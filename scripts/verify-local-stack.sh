...
选项:
  ...
  --web-url <url>              Web URL，full 部署时默认从 .env WEB_PUBLIC_PORT 推导
  --agent-server-url <url>     Agent 连接地址，默认使用 AGENT_PUBLIC_SERVER_URL 或本机中心服务地址
...
stack_mode="$(baize_resolve_stack_mode "$ENV_FILE")"
...
API_URL="${API_URL%/}"
AGENT_SERVER_URL="${AGENT_SERVER_URL:-$(read_env AGENT_PUBLIC_SERVER_URL)}"
if [[ -z "$AGENT_SERVER_URL" || "$AGENT_SERVER_URL" == http://127.0.0.1:* || "$AGENT_SERVER_URL" == http://localhost:* ]]; then
  if baize_stack_has_web "$stack_mode"; then
    AGENT_SERVER_URL="$WEB_URL"
  else
